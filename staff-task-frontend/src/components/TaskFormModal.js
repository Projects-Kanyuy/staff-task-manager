...
const TaskFormModal = ({ isOpen, onClose, onSave, task, users, mode, isStaffMode = false }) => {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', assigneeId: '', dueDate: '', priority: 'medium', startTime: '', endTime: ''
  });
  ...
  useEffect(() => {
    if (task && mode === 'edit') {
      setFormData({
        ...
        assigneeId: task.assigneeIds[0]?._id || '',
      ...
      setFormData({ title: '', description: '', assigneeId: '', dueDate: '', priority: 'medium', startTime: '', endTime: '' });
    }
  }, [task, mode, isOpen]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Ensure assigneeIds is an array, as the backend expects
    onSave({ ...formData, assigneeIds: [formData.assigneeId] });
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {isStaffMode ? (
              <div>
                ...
                <div className="w-full mt-1 px-4 py-2 text-slate-400 bg-slate-800/20 border border-slate-700/50 rounded-lg flex items-center">{user ? user.name : '...'}</div>
              </div>
            ) : (
              <div>
                <label className="block text-sm font-medium text-slate-300">Assign To</label>
                <select name="assigneeId" value={formData.assigneeId} onChange={handleChange} className="w-full mt-1 px-4 py-2 text-slate-200 bg-slate-800/50 border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                  ...
                  {users.map(u => <option key={u._id} value={u._id} className="bg-slate-800">{u.name}</option>)}
                </select>
              </div>
            )}
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-1">
              ...
              <input type="time" name="startTime" value={formData.startTime} onChange={handleChange} className="w-full mt-1 px-4 py-2 text-slate-200 bg-slate-800/50 border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-slate-300">End Time</label>
              <input type="time" name="endTime" value={formData.endTime} onChange={handleChange} className="w-full mt-1 px-4 py-2 text-slate-200 bg-slate-800/50 border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
          </div>
