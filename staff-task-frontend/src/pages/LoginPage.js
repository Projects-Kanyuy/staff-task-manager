      ...
      <div className="w-full max-w-md p-8 space-y-8 bg-black/30 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 mt-1 text-gray-200 bg-gray-900/50 border border-gray-700 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300" required />
          </div>
          <div className="relative">
            <label className="block text-sm font-medium text-gray-300">Password</label>
            <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 mt-1 text-gray-200 bg-gray-900/50 border border-gray-700 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300" required />
        ...
        <p className="text-sm text-center text-gray-400">
          ...
          <Link to="/register" className="font-medium text-blue-400 hover:underline">Register now</Link>
        </p>
      </div>
    </div>
  );
